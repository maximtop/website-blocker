...
import { AMO_REVIEW_NOTES_PATH, GECKO_ID, SOURCE_REQUIRED_FILES } from '../../scripts/deploy/constants';
import {
    releaseVersion,
...
const sourceFiles: Record<string, string> = Object.fromEntries(SOURCE_REQUIRED_FILES.map((file) => [file, 'fixture']));
...
describe('published release contract', () => {
    ...
    it('accepts a Chromium package for Chromium stores and rejects wrong versions and targets', () => {
        ...
        expect(() => verifyManifest(pack({ 'manifest.json': firefoxManifest }), '1.2.3', 'chrome'))
            .toThrow('service worker');
        expect(() => verifyManifest(pack({ 'nested/manifest.json': chromiumManifest }), '1.2.3', 'chrome'))
            .toThrow('exactly one');
    });
    it.runIf(GECKO_ID)('accepts a Firefox package only with the configured Gecko ID', () => {
        ...
        const wrong = JSON.parse(firefoxManifest);
        wrong.browser_specific_settings.gecko.id = 'another-addon@example.test';
        expect(() => verifyManifest(pack({ 'manifest.json': JSON.stringify(wrong) }), '1.2.3', 'firefox'))
            .toThrow('Gecko');
    });
    it('requires matching source and notes for new submissions but permits historical status checks', () => {
        ...
        delete incomplete[SOURCE_REQUIRED_FILES[SOURCE_REQUIRED_FILES.length - 1]];
